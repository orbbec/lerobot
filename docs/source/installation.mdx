...
To install Orbbec camera support:

```bash
pip install 'lerobot[orbbec]'
```

> [!WARNING]
> **Linux — first-time Orbbec setup:** Before using an Orbbec camera for the first time, you must install the udev rules so the device is accessible. Run the following commands once after installing the package (see [pyorbbecsdk installation guide](https://orbbec.github.io/pyorbbecsdk/source/2_installation/install_the_package.html#verify-the-installation) for details):
>
> ```bash
